...
export async function GET(){
 ...
 try{
  ...
  if(!settings)return Response.json({status:'degraded',database:'unreachable',version:VERSION,timestamp:new Date().toISOString()},{status:503,headers:{'Cache-Control':'no-store'}});
  return Response.json({status:settings.maintenance_mode?'maintenance':'ok',database:'connected',version:VERSION,latency_ms:Date.now()-started,features:{registration:settings.beta_registration_open,workspaces:settings.workspace_creation_enabled,store:settings.store_enabled,orby:settings.orby_enabled},settings_updated_at:settings.updated_at,timestamp:new Date().toISOString()},{status:200,headers:{'Cache-Control':'no-store'}});
 }catch{return Response.json({status:'degraded',database:'error',version:VERSION,timestamp:new Date().toISOString()},{status:503,headers:{'Cache-Control':'no-store'}})}
}
